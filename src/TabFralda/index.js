...
export default function TabFralda() {
  ...
  async function getProduto() {
    ...
    query.once("value").then(function (snapshot) {  //percorre com forEach por cada KEY
      snapshot.forEach(function (childSnapshot) {
        ...
        var dataTableData = [];
        if (snapshot.child(key).val().tipo === "fralda" || snapshot.child(key).val().tipo === "Fralda") {
    ...
    return (
      <ScrollView style={styles.container} >
        ...
        <Modal visible={modalCadVisible} animationType="fade" transparent={true} onRequestClose={() => setModalCadVisible(false)} >
          <ViewModalCad fechaModal={setModalCadVisible} preencheTabela={getProduto} selectedPicker={"Fralda"} />
        </ Modal>
